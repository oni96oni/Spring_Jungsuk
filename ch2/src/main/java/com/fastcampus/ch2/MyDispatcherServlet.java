...
// @Controller + @RequestMapping을 합친것이 @WebServlet 서블릿에서는 메서드단위로 맵핑이 안되고 클래스단위로만 맵핑이 된다! 그래서 서블릿은 스프링에비해서 크래스를 많이 만들어야하는단점 보유
...
public class MyDispatcherServlet extends HttpServlet {
	...
	public void service(HttpServletRequest request, HttpServletResponse response) throws IOException {
		Map    map = request.getParameterMap(); //getParameterMap을 썻으니 year, month, day를 map의 형식으로 가져온다
		Model  model = null;
		String viewName = "";
		...
		try {
			...
			Method main = clazz.getDeclaredMethod("main", int.class, int.class, int.class, Model.class);
      			...
      			// 2. main메서드의 매개변수 목록(paramArr)을 읽어서 메서드 호출에 사용할 인자 목록(argArr)을 만든다. 동적으로!
			...
			for(int i=0;i<paramArr.length;i++) {
				...
				if(paramType==Model.class) {
					...
					String strValue = ((String[])value)[0];	// getParameterMap()에서 꺼낸 value는 String배열이므로 변환 필요, 그냥 넣으면 오류발생 Key는 String인데 Value는 String[]이라서
	...
	private void render(Model model, String viewName, HttpServletResponse response) throws IOException {
		...
		PrintWriter out = response.getWriter(); //이렇게하면 콘솔이 아니라 브라우져로 출력
